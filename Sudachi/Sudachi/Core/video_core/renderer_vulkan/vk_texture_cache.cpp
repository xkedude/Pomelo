...
void BlitScale(Scheduler& scheduler, VkImage src_image, VkImage dst_image, const ImageInfo& info,
               VkImageAspectFlags aspect_mask, const YuzuSettings::ResolutionScalingInfo& resolution,
...
TextureCacheRuntime::TextureCacheRuntime(const Device& device_, Scheduler& scheduler_,
    ...
    : device{device_}, scheduler{scheduler_}, memory_allocator{memory_allocator_},
      ...
      render_pass_cache{render_pass_cache_}, resolution{YuzuSettings::values.resolution_info} {
    if (YuzuSettings::values.accelerate_astc.GetValue() == YuzuSettings::AstcDecodeMode::Gpu) {
...
Image::Image(TextureCacheRuntime& runtime_, const ImageInfo& info_, GPUVAddr gpu_addr_,
    ...
    if (IsPixelFormatASTC(info.format) && !runtime->device.IsOptimalAstcSupported()) {
        switch (YuzuSettings::values.accelerate_astc.GetValue()) {
        case YuzuSettings::AstcDecodeMode::Gpu:
            if (YuzuSettings::values.astc_recompression.GetValue() ==
    ...
    if (IsPixelFormatASTC(info.format) && !runtime->device.IsOptimalAstcSupported() &&
        YuzuSettings::values.astc_recompression.GetValue() ==
            YuzuSettings::AstcRecompression::Uncompressed) {
