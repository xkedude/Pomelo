...
void RasterizerVulkan::DrawTexture() {
    ...
    const auto ScaleSrc = [&](auto dim_f) -> s32 {
        ...
        return src_rescaling ? YuzuSettings::values.resolution_info.ScaleUp(dim) : dim;
    };

    const auto ScaleDst = [&](auto dim_f) -> s32 {
        auto dim = static_cast<s32>(dim_f);
        return dst_rescaling ? YuzuSettings::values.resolution_info.ScaleUp(dim) : dim;
    };
...
bool RasterizerVulkan::MustFlushRegion(DAddr addr, u64 size, VideoCommon::CacheType which) {
    ...
    if (!YuzuSettings::IsGPULevelHigh()) {
        return false;
    }
    if (True(which & VideoCommon::CacheType::TextureCache)) {
        std::scoped_lock lock{texture_cache.mutex};
...
void RasterizerVulkan::FlushAndInvalidateRegion(DAddr addr, u64 size,
    ...
    if (YuzuSettings::IsGPULevelExtreme()) {
...
void RasterizerVulkan::UpdateViewportsState(Tegra::Engines::Maxwell3D::Regs& regs) {
    ...
    const float scale = is_rescaling ? YuzuSettings::values.resolution_info.up_factor : 1.0f;
