...
static VkPresentModeKHR ChooseSwapPresentMode(bool has_imm, bool has_mailbox,
    ...
    YuzuSettings::VSyncMode setting = [has_imm, has_mailbox]() {
        // Choose Mailbox or Immediate if unlocked and those modes are supported
        const auto mode = YuzuSettings::values.vsync_mode.GetValue();
        if (YuzuSettings::values.use_speed_limit.GetValue()) {
            return mode;
        }
        switch (mode) {
        ...
        case YuzuSettings::VSyncMode::FifoRelaxed:
            if (has_mailbox) {
                return YuzuSettings::VSyncMode::Mailbox;
            } else if (has_imm) {
                return YuzuSettings::VSyncMode::Immediate;
            }
            [[fallthrough]];
        default:
            return mode;
    ...
    if ((setting == YuzuSettings::VSyncMode::Mailbox && !has_mailbox) ||
        (setting == YuzuSettings::VSyncMode::Immediate && !has_imm) ||
        (setting == YuzuSettings::VSyncMode::FifoRelaxed && !has_fifo_relaxed)) {
        setting = YuzuSettings::VSyncMode::Fifo;
    }
