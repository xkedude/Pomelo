...
Device CreateDevice(const vk::Instance& instance, const vk::InstanceDispatch& dld,
    ...
    const s32 device_index = YuzuSettings::values.vulkan_device.GetValue();
...
RendererVulkan::RendererVulkan(Core::TelemetrySession& telemetry_session_,
    ...
    : RendererBase(emu_window, std::move(context_)), telemetry_session(telemetry_session_),
      ...
      instance(CreateInstance(*library, dld, VK_API_VERSION_1_1, render_window.GetWindowInfo().type,
                              YuzuSettings::values.renderer_debug.GetValue())),
      debug_messenger(YuzuSettings::values.renderer_debug ? CreateDebugUtilsCallback(instance)
      ...
      rasterizer(render_window, gpu, device_memory, device, memory_allocator, state_tracker,
                 scheduler),
      applet_frame() {
    if (YuzuSettings::values.renderer_force_max_clock.GetValue() && device.ShouldBoostClocks()) {
