...
Layer::Layer(const Device& device_, MemoryAllocator& memory_allocator_, Scheduler& scheduler_,
    ...
    if (filters.get_scaling_filter() == YuzuSettings::ScalingFilter::Fsr) {
...
void Layer::SetAntiAliasPass() {
    ...
    const VkExtent2D render_area{
        .width = YuzuSettings::values.resolution_info.ScaleUp(raw_width),
        .height = YuzuSettings::values.resolution_info.ScaleUp(raw_height),
    };

    switch (anti_alias_setting) {
    case YuzuSettings::AntiAliasing::Fxaa:
        anti_alias = std::make_unique<FXAA>(device, memory_allocator, image_count, render_area);
        break;
    case YuzuSettings::AntiAliasing::Smaa:
