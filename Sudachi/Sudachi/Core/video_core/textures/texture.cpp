...
float TSCEntry::MaxAnisotropy() const noexcept {
    ...
    const auto anisotropic_settings = YuzuSettings::values.max_anisotropy.GetValue();
    s32 added_anisotropic{};
    if (anisotropic_settings == YuzuSettings::AnisotropyMode::Automatic) {
        added_anisotropic = YuzuSettings::values.resolution_info.up_scale >>
                            YuzuSettings::values.resolution_info.down_shift;
        added_anisotropic = std::max(added_anisotropic - 1, 0);
    } else {
        added_anisotropic = static_cast<u32>(YuzuSettings::values.max_anisotropy.GetValue()) - 1U;
    }
