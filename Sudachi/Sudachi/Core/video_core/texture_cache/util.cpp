...
u32 CalculateConvertedSizeBytes(const ImageInfo& info) noexcept {
    ...
    if (IsPixelFormatASTC(info.format) && YuzuSettings::values.astc_recompression.GetValue() !=
                                              YuzuSettings::AstcRecompression::Uncompressed) {
        const u32 bpp_div =
            YuzuSettings::values.astc_recompression.GetValue() == YuzuSettings::AstcRecompression::Bc1 ? 2
                                                                                               : 1;
...
void ConvertImage(std::span<const u8> input, const ImageInfo& info, std::span<u8> output,
    ...
    for (BufferImageCopy& copy : copies) {
        ...
        const auto recompression_setting = YuzuSettings::values.astc_recompression.GetValue();
        const bool astc = IsPixelFormatASTC(info.format);

        if (astc && recompression_setting == YuzuSettings::AstcRecompression::Uncompressed) {
            ...
            const auto compress = recompression_setting == YuzuSettings::AstcRecompression::Bc1
                                      ? Tegra::Texture::BCN::CompressBC1
                                      : Tegra::Texture::BCN::CompressBC3;
            const auto bpp_div = recompression_setting == YuzuSettings::AstcRecompression::Bc1 ? 2 : 1;
