...
void EmulatedConsole::ReloadFromSettings() {
    ...
    const auto& player = YuzuSettings::values.players.GetValue()[0];
...
void EmulatedConsole::SetTouchParams() {
    ...
    if (!YuzuSettings::values.mouse_enabled) {
        touch_params[index++] =
    ...
    if (YuzuSettings::values.touch_from_button_maps.empty()) {
        LOG_WARNING(Input, "touch_from_button_maps is unset by frontend config");
        return;
    }

    const auto button_index =
        static_cast<u64>(YuzuSettings::values.touch_from_button_map_index.GetValue());
    const auto& touch_buttons = YuzuSettings::values.touch_from_button_maps[button_index].buttons;
