...
void Mouse::UpdateStickInput() {
    ...
    const float decay_strength = YuzuSettings::values.mouse_panning_decay_strength.GetValue();
    const float decay = 1 - clamped_length * clamped_length * decay_strength * 0.01f;
    const float min_decay = YuzuSettings::values.mouse_panning_min_decay.GetValue();
...
void Mouse::Move(int x, int y, int center_x, int center_y) {
    if (IsMousePanningEnabled()) {
        ...
        const float x_sensitivity =
            YuzuSettings::values.mouse_panning_x_sensitivity.GetValue() * default_panning_sensitivity;
        const float y_sensitivity =
            YuzuSettings::values.mouse_panning_y_sensitivity.GetValue() * default_panning_sensitivity;
        const float deadzone_counterweight =
            YuzuSettings::values.mouse_panning_deadzone_counterweight.GetValue() *
    ...
    if (button_pressed) {
        ...
        const float x_sensitivity =
            YuzuSettings::values.mouse_panning_x_sensitivity.GetValue() * default_stick_sensitivity;
        const float y_sensitivity =
            YuzuSettings::values.mouse_panning_y_sensitivity.GetValue() * default_stick_sensitivity;
...
bool Mouse::IsMousePanningEnabled() {
    ...
    return YuzuSettings::values.mouse_panning && !YuzuSettings::values.mouse_enabled;
}
