...
void TextureCache<P>::CheckFeedbackLoop(std::span<const ImageViewInOut> views) {
    if (!YuzuSettings::values.barrier_feedback_loops.GetValue()) {
        return;
    }
...
void TextureCache<P>::UpdateRenderTargets(bool is_clear) {
    ...
    u32 down_shift = 0;
    if (is_rescaling) {
        up_scale = YuzuSettings::values.resolution_info.up_scale;
        down_shift = YuzuSettings::values.resolution_info.down_shift;
    }
...
bool TextureCache<P>::BlitImage(const Tegra::Engines::Fermi2D::Surface& dst,
    ...
    const auto& resolution = YuzuSettings::values.resolution_info;
...
bool TextureCache<P>::ImageCanRescale(ImageBase& image) {
    ...
    if (YuzuSettings::values.resolution_info.downscale && !image.info.downscaleable) {
        return false;
    }
...
u64 TextureCache<P>::GetScaledImageSizeBytes(const ImageBase& image) {
    const u64 scale_up = static_cast<u64>(YuzuSettings::values.resolution_info.up_scale *
                                          YuzuSettings::values.resolution_info.up_scale);
    const u64 down_shift = static_cast<u64>(YuzuSettings::values.resolution_info.down_shift +
                                            YuzuSettings::values.resolution_info.down_shift);
