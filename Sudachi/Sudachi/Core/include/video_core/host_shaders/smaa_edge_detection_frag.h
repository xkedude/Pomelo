...
constexpr std::string_view SMAA_EDGE_DETECTION_FRAG = {
...
)" R"(layout (location = 1) in vec4 offset[3];
)" R"(
)" R"(layout (location = 0) out vec2 frag_color;
)" R"(
...
)" R"(    frag_color = SMAAColorEdgeDetectionPS(tex_coord, offset, input_tex);
)" R"(}
)" R"(
)" 
};
