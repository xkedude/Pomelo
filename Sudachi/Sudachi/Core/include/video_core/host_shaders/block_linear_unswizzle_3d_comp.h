...
constexpr std::string_view BLOCK_LINEAR_UNSWIZZLE_3D_COMP = {
...
)" R"(layout(binding = BINDING_OUTPUT_IMAGE) uniform writeonly uimage3D output_image;
)" R"(
)" R"(layout(local_size_x = 16, local_size_y = 8, local_size_z = 8) in;
)" R"(
...
)" R"(    offset += (pos.z >> block_depth) * slice_size;
)" R"(    offset += (pos.z & block_depth_mask) << (GOB_SIZE_SHIFT + block_height);
