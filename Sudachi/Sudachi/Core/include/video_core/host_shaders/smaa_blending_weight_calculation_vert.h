...
constexpr std::string_view SMAA_BLENDING_WEIGHT_CALCULATION_VERT = {
...
)" R"(layout (location = 1) out vec2 pix_coord;
)" R"(layout (location = 2) out vec4 offset[3];
)" R"(
...
)" R"(    SMAABlendingWeightCalculationVS(tex_coord, pix_coord, offset);
)" R"(}
)" R"(
)" 
};
