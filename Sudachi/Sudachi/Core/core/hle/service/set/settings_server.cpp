...
Result ISettingsServer::GetLanguageCode(Out<LanguageCode> out_language_code) {
    LOG_DEBUG(Service_SET, "called {}", YuzuSettings::values.language_index.GetValue());

    *out_language_code = available_language_codes[static_cast<std::size_t>(
        YuzuSettings::values.language_index.GetValue())];
    R_SUCCEED();
}
...
Result ISettingsServer::GetRegionCode(Out<SystemRegionCode> out_region_code) {
    ...
    *out_region_code = static_cast<SystemRegionCode>(YuzuSettings::values.region_index.GetValue());
    R_SUCCEED();
}
...
Result ISettingsServer::GetKeyCodeMap(
    ...
    const auto language_code =
        available_language_codes[static_cast<s32>(YuzuSettings::values.language_index.GetValue())];
    const auto key_code =
...
Result ISettingsServer::GetQuestFlag(Out<bool> out_quest_flag) {
    ...
    *out_quest_flag = YuzuSettings::values.quest_flag.GetValue();
    R_SUCCEED();
}
...
Result ISettingsServer::GetDeviceNickName(
    ...
    const std::size_t string_size =
        std::min(YuzuSettings::values.device_name.GetValue().size(), out_device_name->size());

    *out_device_name = {};
    memcpy(out_device_name->data(), YuzuSettings::values.device_name.GetValue().data(), string_size);
    R_SUCCEED();
}
