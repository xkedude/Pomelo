...
void GenerateRandom(std::span<u64> out_random) {
    std::mt19937 rng(YuzuSettings::values.rng_seed_enabled ? YuzuSettings::values.rng_seed.GetValue()
...
void KProcess::LoadModule(CodeSet code_set, KProcessAddress base_addr) {
    ...
    if (this->IsApplication() && YuzuSettings::IsNceEnabled() && patch.size != 0) {
...
#ifdef HAS_NCE
    if (this->IsApplication() && YuzuSettings::IsNceEnabled()) {
