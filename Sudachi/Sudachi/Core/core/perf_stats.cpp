...
PerfStats::~PerfStats() {
    if (!YuzuSettings::values.record_frame_times || title_id == 0) {
        return;
    }
...
void SpeedLimiter::DoSpeedLimiting(microseconds current_system_time_us) {
    if (YuzuSettings::values.use_multi_core.GetValue() ||
        !YuzuSettings::values.use_speed_limit.GetValue()) {
        return;
    }

    auto now = Clock::now();

    const double sleep_scale = YuzuSettings::values.speed_limit.GetValue() / 100.0;
