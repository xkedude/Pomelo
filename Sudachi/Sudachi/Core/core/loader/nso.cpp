...
std::optional<VAddr> AppLoader_NSO::LoadModule(Kernel::KProcess& process, Core::System& system,
    ...
    if (should_pass_arguments && !YuzuSettings::values.program_args.GetValue().empty()) {
        const auto arg_data{YuzuSettings::values.program_args.GetValue()};
    ...
    if (pm && (pm->HasNSOPatch(nso_header.build_id, name) || YuzuSettings::values.dump_nso)) {
