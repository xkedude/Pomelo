...
struct System::Impl {
    ...
    void Initialize(System& system) {
        ...
        is_multicore = YuzuSettings::values.use_multi_core.GetValue();
        extended_memory_layout =
            YuzuSettings::values.memory_layout_mode.GetValue() != YuzuSettings::MemoryLayout::Memory_4Gb;
        ...
        is_async_gpu = YuzuSettings::values.use_asynchronous_gpu_emulation.GetValue();
    ...
    void ReinitializeIfNecessary(System& system) {
        const bool must_reinitialize =
            is_multicore != YuzuSettings::values.use_multi_core.GetValue() ||
            extended_memory_layout != (YuzuSettings::values.memory_layout_mode.GetValue() !=
                                       YuzuSettings::MemoryLayout::Memory_4Gb);
    ...
    void RefreshTime(System& system) {
        ...
        auto new_name = YuzuSettings::GetTimeZoneString(YuzuSettings::values.time_zone_index.GetValue());
        std::memcpy(name.data(), new_name.data(), std::min(name.size(), new_name.size()));

        timezone_service->SetDeviceLocationName(name);

        u64 time_offset = 0;
        if (YuzuSettings::values.custom_rtc_enabled) {
            time_offset = YuzuSettings::values.custom_rtc_offset.GetValue();
        }
    ...
    SystemResultStatus SetupForApplicationProcess(System& system, Frontend::EmuWindow& emu_window) {
        ...
        if (YuzuSettings::values.enable_renderdoc_hotkey) {
    ...
    SystemResultStatus Load(System& system, Frontend::EmuWindow& emu_window,
        ...
        if (YuzuSettings::values.gamecard_inserted) {
            if (YuzuSettings::values.gamecard_current_game) {
                fs_controller.SetGameCard(GetGameFileFromPath(virtual_filesystem, filepath));
            } else if (!YuzuSettings::values.gamecard_path.GetValue().empty()) {
                const auto& gamecard_path = YuzuSettings::values.gamecard_path.GetValue();
...
void System::InitializeDebugger() {
    impl->InitializeDebugger(*this, YuzuSettings::values.gdbstub_port.GetValue());
}
...
bool System::DebuggerEnabled() const {
    return YuzuSettings::values.use_gdbstub.GetValue();
}
